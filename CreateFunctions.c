...
char *fieldToChar(Field *f)
{
    static char res[SMALL_BUFFER];
    memset(res,0,SMALL_BUFFER);
    // NAME;TYPE;SIZE;PK?;NN?;AI;UNQ;FK?;
    char tmp[TINY_BUFFER]={0};

    formatToCSV(res,f->name,0);
    formatToCSV(res,f->type,0);    
    sprintf(tmp,"%d",f->size);
    formatToCSV(res,tmp,0);
    sprintf(tmp,"%d",f->isPrimaryKey);
    formatToCSV(res,tmp,0);
    sprintf(tmp,"%d",f->isNotNull);
    formatToCSV(res,tmp,0);
    sprintf(tmp,"%d",f->isAutoIncrement);
    formatToCSV(res,tmp,0);
    sprintf(tmp,"%d",f->isUnique);
    formatToCSV(res,tmp,0);
    formatToCSV(res,f->foreignKey,1);
